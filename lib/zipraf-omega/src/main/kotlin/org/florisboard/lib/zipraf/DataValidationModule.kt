...
class DataValidationModule {
    ...
    data class ValidationRule<T>(
        ...
        val errorMessage: String,
        val severity: ValidationStatus = ValidationStatus.ERROR
    )
            ...
            if (!typedInvariant.condition(data)) {
                errors.add(
                    ValidationError(
                        ...
                        severity = ValidationStatus.ERROR.name
                    )
                )
            }
        }
    ...
    fun validateLogic(
        ...
        return LogicValidationResult(
            validationName = name,
            result = if (allPassed) ValidationStatus.VALID.name else ValidationStatus.INVALID.name,
